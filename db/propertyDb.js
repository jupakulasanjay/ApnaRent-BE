...
function buildPublicPropertiesWhere({
  ...
  if (centroid && radiusKm) {
    ...
    // Fuzzy fallback — see db/listingDb.js for rationale.
...
export async function searchPublicProperties({
  ...
  // property_type stays a hard filter — plot/villa/apartment are categorically
  // distinct asset classes, so a user asking for "plot" really does not want
  // a villa. The Claude prompt was updated to stop inferring property_type
  // from BHK, so this only triggers when the user explicitly named the type.
  if (property_type) {
    where.push(`property_type = $${i++}`);
    params.push(property_type);
  }
