...
before(async () => {
  ...
  await req("POST", `/api/rentals/${ctx.listingActive}/submit`, {
    token: ctx.owner,
  });
  await req("POST", `/api/admin/rentals/${ctx.listingActive}/approve`, {
    token: ctx.admin,
  });
  ...
  await req("POST", `/api/rentals/${ctx.listingActive2}/submit`, {
    token: ctx.owner,
  });
  await req("POST", `/api/admin/rentals/${ctx.listingActive2}/approve`, {
    token: ctx.admin,
  });
});
...
test("POST without listing_id → 400", async () => {
...
test("GET returns own interests ordered created_at DESC, scoped to user", async () => {
  ...
  const { status, json } = await req("GET", "/api/interests", {
    token: ctx.tenant,
  });
  assert.equal(status, 200);
  ...
  for (const entry of json) {
    assert.equal(entry.kind, "listing");
    assert.ok(Array.isArray(entry.listing.images));
  }
});
...
test("DELETE /api/interests/listing/:id → 204 and removes from GET", async () => {
  const del = await req(
    "DELETE",
    `/api/interests/listing/${ctx.listingActive}`,
    { token: ctx.tenant },
  );
  assert.equal(del.status, 204);

  const get = await req("GET", "/api/interests", { token: ctx.tenant });
  const targetIds = get.json.map((r) => r.target_id);
  assert.ok(!targetIds.includes(ctx.listingActive));
  assert.ok(targetIds.includes(ctx.listingActive2));
});
...
test("DELETE with bad kind → 400", async () => {
  const { status } = await req("DELETE", `/api/interests/garbage/1`, {
    token: ctx.tenant,
  });
  assert.equal(status, 400);
});
