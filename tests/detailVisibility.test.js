...
before(async () => {
  ...
  const mkListing = async (title) => {
    const { json } = await req("POST", "/api/rentals", {
      ...
      body: {
        ...
        address: `addr for ${title}`, city: "Bangalore"
      }
    })
    return json.id
  }
  ...
  // --- Property: one draft for owner A (to verify the same rules apply to properties) ---
  const { json: pDraft } = await req("POST", "/api/properties", {
    token: ctx.ownerA,
    body: {
      title: "draft property A", price: 6000000, property_type: "villa",
      address: "p addr", city: "Bangalore"
    }
  })
  ctx.propertyDraftA = pDraft.id
})
...
test("same visibility rules apply to properties: owner sees own draft, tenant gets 404", async () => {
  const ownerCall = await req("GET", `/api/properties/${ctx.propertyDraftA}`, { token: ctx.ownerA })
  assert.equal(ownerCall.status, 200)
  assert.equal(ownerCall.json.status, "draft")

  const tenantCall = await req("GET", `/api/properties/${ctx.propertyDraftA}`, { token: ctx.tenant })
  assert.equal(tenantCall.status, 404)
