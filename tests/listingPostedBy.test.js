...
async function registerAndLogin(email, role, name) {
  await req("POST", "/api/auth/register", {
    body: { email, password: "password123", role, name },
  });
...
before(async () => {
  ...
  ctx.owner = await registerAndLogin(
    "owner-listpb@test.com",
    "owner",
    "Lara Lister",
  );
  ctx.admin = (
...
test("POST /api/rentals/:id/submit response includes posted_by", async () => {
  const { status, json } = await req(
    "POST",
    `/api/rentals/${ctx.listingId}/submit`,
    { token: ctx.owner },
  );
  assert.equal(status, 200);
...
test("GET /api/admin/rentals/pending → each item carries posted_by", async () => {
  const { status, json } = await req("GET", "/api/admin/rentals/pending", {
    token: ctx.admin,
  });
  assert.equal(status, 200);
...
test("POST /api/admin/rentals/:id/approve → response includes posted_by", async () => {
  const { status, json } = await req(
    "POST",
    `/api/admin/rentals/${ctx.listingId}/approve`,
    { token: ctx.admin },
  );
  assert.equal(status, 200);
...
test("GET /api/rentals/owned → posted_by present on each item", async () => {
  const { status, json } = await req("GET", "/api/rentals/owned", {
    token: ctx.owner,
  });
  assert.equal(status, 200);
  for (const l of json.data) {
...
// Landing-page hero sends raw user queries to /api/search/rentals. Short
// single-token queries must come back 200 with whatever filters could be
// parsed — never 400 — so the hero spinner doesn't dead-end.
test("POST /api/search/rentals with single-token query → 200, filters object present", async () => {
  const { status, json } = await req("POST", "/api/search/rentals", {
    body: { query: "Indiranagar" },
  });
  assert.equal(status, 200);
  assert.ok(json.filters && typeof json.filters === "object");
  assert.ok(Array.isArray(json.results));
  // results may be empty in the test DB; suggestion_reason is "no-exact-matches"
  // only if any suggestions were collected.
});

test("POST /api/search/rentals with unparseable query → 200, no error", async () => {
  const { status, json } = await req("POST", "/api/search/rentals", {
    body: { query: "asdfqwerzxcv" },
  });
  assert.equal(status, 200);
  assert.ok(json.filters && typeof json.filters === "object");
  assert.ok(Array.isArray(json.results));
...
//
// Listings don't currently have an admin-create flow on the routes, so we
// exercise the resolver via a pending_verification listing whose owner_id is
// overridden in-DB to point at the admin user. This keeps the test about the
// resolver, not the routing.
test("listing whose owner_id is the admin user → posted_by = ApnaRent", async () => {
  ...
  try {
    ...
    await client.query(`UPDATE listings SET owner_id = $1 WHERE id = $2`, [
      adminId,
      listingId,
    ]);
    await client.query(`UPDATE listings SET status = 'active' WHERE id = $1`, [
      listingId,
    ]);
