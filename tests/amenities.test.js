...
test("Flattened list dedupes slugs that appear in multiple categories", async () => {
  ...
  assert.equal(
    slugs.length,
    uniq.size,
    `duplicate slugs in flat list: ${slugs.length} vs ${uniq.size}`,
  );
});
...
test("Listing supports amenities[] on create + update", async () => {
  ...
  assert.deepEqual(updated.json.amenities, [
    "gym",
    "swimming_pool",
    "Custom: rooftop terrace",
  ]);
});

test("Listing with custom amenities round-trips through GET", async () => {
  const created = await req("POST", "/api/rentals", {
    token: ctx.owner,
    body: {
      ...
      amenities: [
        "private_garden_lawn",
        "swimming_pool",
        "Custom: rooftop solar farm",
      ],
    },
  });
  assert.equal(created.status, 201);
  assert.deepEqual(created.json.amenities, [
    "private_garden_lawn",
    "swimming_pool",
    "Custom: rooftop solar farm",
  ]);

  const fetched = await req("GET", `/api/rentals/${created.json.id}`, {
    token: ctx.owner,
  });
...
test("Listing created without amenities defaults to empty array", async () => {
...
test("Validator rejects amenity strings beyond max length", async () => {
  ...
  const { status } = await req("POST", "/api/rentals", {
    token: ctx.owner,
    body: {
      title: "x",
      rent: 1,
