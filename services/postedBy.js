// Single source of truth for the `posted_by` label that listings and
// properties surface to the FE. The rule:
//
//   admin owner            → "ApnaRent"
//   non-admin owner w/ name → owner.name
//   non-admin owner, no name → "Owner"
//
// Both domains use the same resolver — keep them aligned by editing only
// this file.
...
export async function decoratePostedByMany(records) {
  ...
  return records.map((r) => ({ ...r, posted_by: map.get(r.owner_id) || "Owner" }))
}
