import {
  getLocalityCentroid,
  upsertLocality,
} from "../../db/localities/localityDb.js";

const GEOCODE_URL = "https://maps.googleapis.com/maps/api/geocode/json";
const GEOCODE_REGION = "India";
const GEOCODE_SOURCE = "google";

// Resolves (city, locality) to a centroid. Cache hit returns immediately; cache
// miss calls Google Geocoding once and writes the result back. Returns null
// when no API key is set, the API errors, or no result — callers fall back to
// literal locality ILIKE.
export async function resolveLocalityCentroid(city, locality) {
  ...
  const address = [locality, city, GEOCODE_REGION].filter(Boolean).join(", ");
